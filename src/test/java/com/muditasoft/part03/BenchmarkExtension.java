...
public class BenchmarkExtension implements BeforeAllCallback, AfterAllCallback, BeforeTestExecutionCallback, AfterTestExecutionCallback {
    ...
    public void afterAll(ExtensionContext context) throws Exception {
        LocalDateTime startTime = getStoreForContainer(context).remove(START_TIME, LocalDateTime.class);
        long runTime = startTime.until(LocalDateTime.now(), ChronoUnit.MILLIS);
    ...
    public void afterTestExecution(ExtensionContext context) throws Exception {
        LocalDateTime startTime = getStoreForMethod(context).remove(START_TIME, LocalDateTime.class);
        long runTime = startTime.until(LocalDateTime.now(), ChronoUnit.MILLIS);
